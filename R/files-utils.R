...
#' @examples \dontrun{
#' }
read_sysfile <- function(..., package = "mwr"){
  if (is.null(package)){
    path = file.path(...)
  } else {
...
#' @examples \dontrun{
#' }
read_csv_sysfile <- function(path){
  read.csv(sysfile(path),stringsAsFactors = FALSE)
}
...
#' @examples \dontrun{
#' }
sysfile <- function(..., package = "ciudatos"){
  if (is.null(package)){
    path = file.path(...)
  } else {
    path = system.file(..., package = package)
